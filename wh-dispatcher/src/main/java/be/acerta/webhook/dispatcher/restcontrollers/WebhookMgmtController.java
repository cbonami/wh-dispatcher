...
public class WebhookMgmtController {
        ...
        //
        //
        @Operation(summary = "Register a webhook (URL) with a unique name", description = "Used by an external party (\'application\') to register a url to which the service will dispatch (POST) messages", tags = {"webhook"})
        @ApiResponses({@ApiResponse(responseCode = "201", description = "successful operation", content = @Content(schema = @Schema(implementation = Webhook.class)))})
        ...
        public ResponseEntity<Webhook> newWebhook(@RequestBody @Valid NewWebhookDto dto) {
                Webhook applicationRequest =  //
                //
                //
                Webhook.builder().name(dto.getName()).url(dto.getUrl()).pubSub(dto.isPubSub()).build();
        ...
        @ApiResponses({@ApiResponse(responseCode = "200", description = "successful operation", content = @Content(array = @ArraySchema(schema = @Schema(implementation = Webhook.class))))})
        ...
        public ResponseEntity<CollectionModel<Webhook>> listWebhooks() {
                ...
                //
                //
                linkTo(methodOn(WebhookMgmtController.class).listWebhooks()).withSelfRel().andAffordance(afford(methodOn(WebhookMgmtController.class).newWebhook(null))).andAffordance(afford(methodOn(WebhookMgmtController.class).newMessage(null, null, "none", null)))));
        }

        //
        //
        @Operation(summary = "Returns an webhook by name", tags = {"webhook"})
        @ApiResponses({@ApiResponse(responseCode = "200", description = "successful operation", content = @Content(schema = @Schema(implementation = Bucket.class)))})
        ...
        //
        //
        @Operation(summary = "List all logical buckets, and the messages that they contain, that are currently being processed for a given webhook.", description = "Plz note that the results evaporate i.e. will vary from msec to msec as buckets (mini-queues) are emptied.", tags = {"webhook"})
        @ApiResponses({@ApiResponse(responseCode = "200", description = "successful operation", content = @Content(array = @ArraySchema(schema = @Schema(implementation = Bucket.class))))})
        ...
        @PathVariable("whName") String whName) {
                ...
                webhookRepository.findByName(whName).map(wh -> {
                        ...
                        return ResponseEntity.ok(CollectionModel.of(bucketResources, linkTo(methodOn(WebhookMgmtController.class).listBuckets(whName)).withSelfRel()));
                }).orElse(ResponseEntity.notFound().build());
        }
        ...
        public ResponseEntity<CollectionModel<Bucket>> listMessages(@PathVariable("whName") String whName) {
                ...
                webhookRepository.findByName(whName).map(wh -> {
                        List<Bucket> bucketResources = getBucketsByWebhookId(wh.getId());
                        return ResponseEntity.ok(CollectionModel.of(bucketResources, linkTo(methodOn(WebhookMgmtController.class).listMessages(whName)).withSelfRel()));
                }).orElse(ResponseEntity.notFound().build());
        }
        ...
        @PathVariable("bucketId") String bucketId) {
                ...
                webhookRepository.findByName(whName).map(wh -> {
                        ...
                        bucket.add(linkTo(methodOn(WebhookMgmtController.class).forceProcessing(whName, bucketId)).withRel("force"));
                        bucket.add(linkTo(methodOn(WebhookMgmtController.class).triggerProcessing(whName, bucketId)).withRel("trigger"));
        ...
        private void removeMessagesByWebhookId(String whId) {
                redisClient.getBuckets().keySet().stream().forEach(key -> {
                        try {
                                if (key.startsWith(whId)) redisClient.removeBucket(key);
                        } catch (Exception e) {
        ...
        private List<Message> getMessagesInBucketById(String absBucketId) {
                return //
                //
                redisClient.getBuckets().getAll(absBucketId).stream().map(message -> JsonUtil.jsonToObject(message, Message.class)).collect(Collectors.toList());
        }
        ...
        private List<Bucket> getBucketsByWebhookId(String whId) {
                ...
                //
                //
                StreamSupport.stream(this.redisClient.getBuckets().keySet().spliterator(), false).filter(bucketId -> bucketId.startsWith(whId)).map(bucketId -> {
                        List<Message> messages = getMessagesInBucketById(bucketId);
                        Bucket bucket = Bucket.builder().id(bucketId).messages(messages).build();
                        bucket.add(linkTo(methodOn(WebhookMgmtController.class).getBucket(whId, bucket.getId())).withSelfRel());
                        return bucket;
                }).collect(Collectors.toList());
        }
        ...
        public ResponseEntity<Void> removeBucket(@PathVariable("whName") String whName, @PathVariable("bucketId") String bucketId) {
                return webhookRepository.findByName(whName).map(wh -> {
                        String absBucketId = transformToAbsoluteBucketId(wh.getId(), bucketId);
        ...
        public ResponseEntity<Void> triggerProcessing(@PathVariable("whName") String whName, @PathVariable("bucketId") String bucketId) {
                return webhookRepository.findByName(whName).map(wh -> {
                        String absBucketId = transformToAbsoluteBucketId(wh.getId(), bucketId);
        ...
        public ResponseEntity<BooleanDto> isProcessorLocked(@PathVariable("whName") String whName, @PathVariable("bucketId") String bucketId) {
                return webhookRepository.findByName(whName).map(wh -> {
                        String absBucketId = transformToAbsoluteBucketId(wh.getId(), bucketId);
                        redisClient.triggerProcessing(absBucketId);
        ...
        public ResponseEntity<Void> forceProcessing(@PathVariable("whName") String whName, @PathVariable("bucketId") String bucketId) {
                return webhookRepository.findByName(whName).map(wh -> {
                        String absBucketId = transformToAbsoluteBucketId(wh.getId(), bucketId);
        ...
        //
        //
        @Operation(summary = "Delete webhook by name.", description = "Deletes an webhook and deregisters all its webhooks.", tags = {"webhook"})
        ...
        //
        //
        @Operation(summary = "POST a message to specified registered webhook (url).", description = "Mainly geared towards testing of the webhook-api of the destination-application." + " Plz note that the webhook dispatcher is kept as generic as possible, which means" + "that it is up to the poster to define the bucket that a message belongs to.", tags = {"message"})
        ...
        @RequestHeader(value = "Content-Type", defaultValue = APPLICATION_JSON_VALUE) String mediaType) {
                ...
                webhookRepository.findByName(whName).map(webhook -> {
                        log.debug("Publishing Message [{}] to existing Webhook {}", lazy(message::toString), lazy(webhook::getName));
                        final String id = UUID.randomUUID().toString();
                        final String idempotencyKey = "" + message.hashCode();
                        WebhookMessageDto webhookMessageDto = WebhookMessageDto.builder().id(id).data(idempotencyKey).type(message.getType()).data(message.getData()).idempotencyKey(idempotencyKey).mediaType(isEmpty(mediaType) ? APPLICATION_JSON_VALUE : mediaType).webhookUrl(webhook.getUrl()).delivery(MessageDeliveryType.WEBHOOK_V1.toString()).build();
                        Message msg = this.webhookRedisMessageProducer.asyncSend(webhook.getId(), String.valueOf(bucketId.equals("none") ? randomBucketNumber(30) : transformToAbsoluteBucketId(webhook.getId(), bucketId)), webhookMessageDto);
        ...
        @DeleteMapping(value = "/api" + WEBHOOKS_URL + "/{whName}/buckets/{bucketId}/{messageId}", produces = HAL_JSON_VALUE)
        public ResponseEntity<Void> deleteMessage(@PathVariable("whName") String whName, @PathVariable("bucketId") String bucketId, String messageId) {
                return webhookRepository.findByName(whName).map(wh -> {
                        String absBucketId = transformToAbsoluteBucketId(wh.getId(), bucketId);
        ...
        public static class Endpoint extends RepresentationModel<Endpoint> {
                ...
                public boolean equals(final Object o) {
                        ...
                        if (!(o instanceof WebhookMgmtController.Endpoint)) return false;
        ...
        public static class EmptyResource extends RepresentationModel<EmptyResource> {
                ...
                public boolean equals(final Object o) {
                        if (o == this) return true;
                        if (!(o instanceof WebhookMgmtController.EmptyResource)) return false;
        ...
        public ResponseEntity<RedisInfoDto> getInfo() {
                return ok(redisStatusDto().withRedisStatus(Lists.newArrayList(redisClient).stream().map(cl -> redisGroupInfoDto().withAantalBuckets(redisClient.getBuckets().keySet().size()).withBucketIds(redisClient.getBuckets().keySet()).withAantalWachtendeBuckets(redisClient.getAwaitRetries().keySet().size()).withWachtendeBuckets(redisClient.getAwaitRetries().keySet())).collect(toList())));
        }
